...
ADIS16470_IMU::ADIS16470_IMU(IMUAxis yaw_axis, SPI::Port port, ADIS16470CalibrationTime cal_time) : 
  ...
  std::cout << "DEC_RATE Preconfig: " << ReadRegister(DEC_RATE) << std::endl;
  ...
  FRC_ReportError(warn::Warning, "{} {}", "ADIS16470 IMU Detected. Starting initial calibration delay.", units::second_t(pow(2, m_calibration_time)) / 2000 * 64 * 1.1);
  ...
  FRC_ReportError(warn::Warning, "{}", "Finished calibration delay, now WriteRegister(GLOB_CMD, 0x0001);");
  ...
  FRC_ReportError(warn::Warning, "{}", "Finishe WriteRegister(GLOB_CMD, 0x0001);, now SwitchToAutoSPI()");
